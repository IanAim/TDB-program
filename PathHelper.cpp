...
//相对路径转换成绝对路径文件名
 ...
 int __cdecl DebugPrint(const char* szFormat, ...)
 {
     ...
     _snprintf_s(szMoreInfo, nBufSize, nBufSize-1, "TID:%04d, Time: %02d:%02d-%02d-%03d: %s", GetCurrentThreadId(), st.wHour, st.wMinute, st.wSecond, st.wMilliseconds, szBuf);
 ...
 int ErrPrint(const char* szFormat, ...)
 {
     ...
     _snprintf_s(szMoreInfo, nBufSize, nBufSize-1, "TID:%04ld, Time: %02d:%02d-%02d-%03d: %s", GetCurrentThreadId(), timeInfo.nHour, timeInfo.nMinute, timeInfo.nSecond, timeInfo.nMiniSecond, szBuf);
	 
#ifdef __PLATFORM_WINDOWS__ 
     DebugPrint(szMoreInfo);
     fprintf(stderr, (const char*)szMoreInfo);
#else
     fprintf(stderr, (const char*)szMoreInfo);
#endif
     delete [] szMoreInfo;
 ...
 unsigned int DateTimeInfo::ToYYMMDD()
 {
